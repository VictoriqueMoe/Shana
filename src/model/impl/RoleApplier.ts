...
import {RolePersistenceModel} from "../DB/entities/autoMod/impl/RolePersistence.model.js";
import {DbUtils} from "../../utils/Utils.js";
import {MemberRoleChange} from "./MemberRoleChange.js";
import {LogChannelManager} from "../framework/manager/LogChannelManager.js";

@singleton()
export class RoleApplier extends DataSourceAware {

    public constructor(private _logChannelManager: LogChannelManager) {
        super();
    }
    ...
    public async roleLeaves(role: Role, member: GuildMember, model: typeof RolePersistenceModel): Promise<RolePersistenceModel> {
        ...
        const userId = member.user.id;
        await this._logChannelManager.postToLog(`member ${member.user.tag} left the guild while having the role of ${role.name}`, guildId);
        return DbUtils.build(model, {
            userId,
            roleId,
            guildId
        });
